...
*   从 [Apache ECharts 官网下载界面](.//zh/download.html) 获取官方源码包后构建。
...
最简单的更改全局样式的方式，是直接采用颜色主题（theme）。例如，在 [示例集合](.//examples/zh/index.html) 中，可以选择 “Theme”，直接看到采用主题的效果。
...
**更多的自定义系列的例子参见：[custom examples](.//examples/zh/index.html#chart-type-custom)**
...
*   如果 [在线自定义构建 ECharts](.//zh/builder.html)，则需要勾上页面下方的 “SVG 渲染”。
...
在看完文章之后，你可以前往 [官方示例](.//examples/zh/index.html#chart-type-globe) 和 [Gallery](https://gallery.echartsjs.com/explore.html#tags=echarts-gl) 去了解更多使用 GL 制作的示例，对于文章中我们没法解释到的代码，也可以前往 [GL 配置项手册](option-gl.html) 查看具体的配置项使用方法。

## 如何下载和引入 ECharts GL

为了不再增加已经很大了的 ECharts 完整版的体积，我们将 GL 作为扩展包的形式提供，和诸如水球图这样的扩展类似，如果要使用 GL 里的各种组件，只需要在引入`echarts.min.js`的基础上再引入一个`echarts-gl.min.js`。你可以从 [官网](.//zh/download.html) 下载最新版的 GL，然后在页面中通过标签引入：
...
可以先从 .//examples/data/asset/data/life-expectancy-table.json 获取这份数据。
...
`ec-canvas` 目录下有一个 `echarts.js`，默认我们会在每次 `echarts-for-weixin` 项目发版的时候替换成最新版的 ECharts。如有必要，可以自行从 ECharts 项目中下载[最新发布版](https://github.com/apache/echarts/releases)，或者从[官网自定义构建](.//builder.html)以减小文件大小。
